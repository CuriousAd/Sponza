      ...
      if ((obj as THREE.Mesh).isMesh) {
        ...
        if (typeof mesh.material === "object" && mesh.material !== null) {
          Object.keys(mesh.material).forEach((key) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const matProp = (mesh.material as any)[key];
            if (
              matProp !== null &&
...
export default function ASCIIText({
  ...
  useEffect(() => {
    ...
    const createAndInit = async (element: HTMLDivElement, width: number, height: number) => {
    ...
    const observeResize = (element: HTMLDivElement) => {
        ...
        observer = new IntersectionObserver(
          async ([entry]) => {
            ...
            if (
              ...
              const instance = await createAndInit(
                container,
