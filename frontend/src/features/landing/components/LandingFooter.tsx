...
export function LandingFooter({ className }: LandingFooterProps) {
  ...
  return (
    <footer
      className={cn(
        "bg-sponza-900 border-t border-sponza-700/30 py-10 px-6",
        className
      )}
    >
      <div className="max-w-5xl mx-auto">
        ...
        >
          {LANDING_FOOTER_LINKS.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="text-sponza-600 hover:text-sponza-300 text-sm transition-colors duration-200"
            >
              {link.label}
            </Link>
          ))}
        ...
        <p className="text-center text-sponza-700 text-xs">
          &copy; {year} Sponza. All rights reserved.
        </p>
      </div>
    </footer>
  );
